...
const StoredViewer = () => {
  ...
  useEffect(() => {
    // if we have a storedData in localStorage, we want to use that data
    const storedData = localStorage.getItem("storedData");
    const fetchStoredData = async (data) => {
        try {
            setData(JSON.parse(data));
            const initialExpandedGameplan = {};
        JSON.parse(storedData).matchups.forEach((_, index) => {
            if (JSON.parse(storedData).matchups[index].gameplans.length > 0) {
                initialExpandedGameplan[index] = 0;
            }
        });
        setExpandedGameplan(initialExpandedGameplan);

        await fetchPokemonImages(JSON.parse(storedData).paste);
        } catch (error) {
  ...
  if (!data) {
    return (
      ...
      >
        <h1 className="text-center mb-4">{pageTitle}</h1>
        <p className="text-center">You don't have any stored data. Go back to the homepage and use "Save Locally" to use this page.</p>
      </div>
    );
  }
      ...
      >
        <div className="card-body d-flex justify-content-between">
          ...
          <div>
            ...
            <button onClick={handleCleanStorage} className="btn btn-danger ms-2">
                Clean Storage
            </button>
          </div>
        </div>
